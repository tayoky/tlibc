...
#define strto(type, name, max, min) \
	type name(const char *str, char **end, int base) { \
		signed char sign = 1; \
		type integer = 0; \
		if (end) *end = (char *)str; \
\
...
// it support both . and ,
#define strtd(type, name) \
	type name(const char *str, char **end) { \
		long long integer = 0; \
		long long pow = 0; \
		signed char sign = 1; \
		if (end) *end = (char *)str; \
